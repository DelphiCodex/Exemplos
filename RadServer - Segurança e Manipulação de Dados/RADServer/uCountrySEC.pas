...
type
  ...
  published
    ...
    [ResourceSuffix('customer/{customer_id}/endereco/{address_id}')]
    procedure GetItemFiltered(const AContext: TEndpointContext;
      const ARequest: TEndpointRequest; const AResponse: TEndpointResponse);
...
procedure TCountryResource1.GetItemFiltered(const AContext: TEndpointContext;
  const ARequest: TEndpointRequest; const AResponse: TEndpointResponse);
var
  LJSON: TJSONArray;

  LResponse, LAddressID,
  LCustomerId, LAddressCustomerId : String;
begin
//  CheckSecurity(AContext, ARequest, AResponse);

  ARequest.Params.TryGetValue('customer_id', LCustomerId);
  ARequest.Params.TryGetValue('address_id',  LAddressId);

(*
  LJSON := TJSONArray.Create;
  LJSON.Add(MakeJSON('customer_id', LCustomerId));
  LJSON.Add(MakeJSON('address_id', LAddressId));

  AResponse.Body.SetValue(LJSON, True);
*)

  AResponse.Body.SetValue(TJSONString.Create('customer_id: ' + LCustomerId + ' - ' +
                                             'address_id: '  + LAddressId), True);
end;
